...
class Estimator:
    ...
    def load(self, what):
        ...
        if self.__class__.__name__ != state["class"]:
            raise RuntimeError(
                "This Estimator is {}, cannot load {}".format(self.__class__.__name__, state["class"])
            )
        if self.layer.__class__.__name__ != state["layer_class"]:
            raise RuntimeError(
                "This Layer is {}, cannot load {}".format(self.layer.__class__.__name__, state["layer_class"])
            )
        self.N = state["N"]
        self.S = state["S"]
        self.M = state["M"]
...
class IBAInterpreter:
    ...
    def vision_heatmap(self, text_t, image_t, precomputed_text_feature=False):
        saliency, loss_c, loss_f, loss_t = self._run_vision_training(
            text_t,
            image_t,
    ...
    def _run_text_training(self, text_t, image_t):
        ...
        loss_c, loss_f, loss_t = self._train_bottleneck(text_t, image_t, precomputed_text_feature=False)
    ...
    def _run_vision_training(self, text_t, image_t, precomputed_text_feature=False):
        replace_layer(self.model.vision_model, self.original_layer, self.sequential)
        loss_c, loss_f, loss_t = self._train_bottleneck(
            text_t,
            image_t,
            precomputed_text_feature=precomputed_text_feature,
        )
    ...
    def _encode_text(self, text_batch, precomputed_text_feature=False):
        if precomputed_text_feature:
            return text_batch
        return self.model.get_text_features(text_batch)

    def _train_bottleneck(self, text_t: torch.Tensor, image_t: torch.Tensor, precomputed_text_feature=False):
        # Router 妯″紡涓� text_t 宸茬粡鏄� (1, D) 鏂囨湰鐗瑰緛; 鏃фā寮忎笅鏄� (1, L) token ids
        if (not self.ensemble) or precomputed_text_feature:
            ...
            for _ in tqdm(range(self.train_steps), desc="Training Bottleneck", disable=not self.progbar):
                optimizer.zero_grad()
                text_output = self._encode_text(batch_text, precomputed_text_feature=precomputed_text_feature)
                image_output = self.model.get_image_features(batch_image)
                loss_c, loss_f, loss_t = self.calc_loss(outputs=text_output, labels=image_output)
                loss_t.backward()
            ...
            for _ in tqdm(range(self.train_steps), desc="Training Bottleneck", disable=not self.progbar):
                ...
                loss_c, loss_f, loss_t = self.calc_loss(
                    outputs=text_outputs.expand(self.batch_size, -1),
                    labels=image_output,
                )
                loss_t.backward()
                optimizer.step(closure=None)
